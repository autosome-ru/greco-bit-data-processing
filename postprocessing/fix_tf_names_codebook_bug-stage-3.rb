...
def motif_similarity_rename_pairs(datasets, renames)
  ...
  Dir.glob("artifact_sims_precise/*").map{|fn|
    ...
    if renames_for_motif.size == 0
      [fn, File.join('artifact_sims_precise_recalc', File.basename(fn))]
    elsif renames_for_motif.size == 1
      rename_info = renames_for_motif.take_the_only
      new_basename = renamed_motif_basename_by_info(fn, rename_info)
...
copy_files(motif_similarity_rename_pairs(datasets_full, renames), symlink: true)
