BN_PATTERN = /^(?<tf>[^.@]+\.[^.@]+)@(?<exp_type>HTS\.IVT|HTS\.Lys|HTS\.GFPIVT|AFS\.IVT|AFS\.Lys|AFS\.GFPIVT|PBM\.ME|PBM\.HK|SMS|CHS)@(?<dataset>[-+a-z]+)@(?<tool>[^@.]+\.[^@.]+)@(?<motif_subname>[^@.]+)$/
PPM_TOLERANCE = 0.05
...
filelist.select{|fn|
  ...
  report << "Filename doesn't match pattern tf.construct@{PBM.HK,PBM.ME,CHS,SMS,HTS.IVT,HTS.Lys,HTS.GFPIVT,AFS.Lys,AFS.IVT,AFS.GFPIVT}@dataset_id@team.tool@motif_name_wo_dots"  unless bn_wo_ext.match(BN_PATTERN)
  ...
  begin
    ...
    report << "Format motifs in non-scientific format"  unless matrix_rows.all?{|row| row.all?{|el| el.match?(/^[-+]?\d+(\.\d+)?$/) } }
