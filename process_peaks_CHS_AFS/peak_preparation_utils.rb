...
def store_train_val_stats(tf_infos, filename, experiment_by_peak_id)
  File.open(filename, 'w') {|fw|
    ...
    tf_infos.each{|tf_info|
      ...
      Dir.glob("#{RESULTS_FOLDER}/train_intervals/#{tf_info[:tf]}.*.train.interval").each{|train_fn|
      ...
      Dir.glob("#{RESULTS_FOLDER}/validation_intervals/#{tf_info[:tf]}.*.basic_val.interval").each{|basic_validation_fn|
      ...
      Dir.glob("#{RESULTS_FOLDER}/validation_intervals/#{tf_info[:tf]}.*.advanced_val_*.interval").each{|advanced_validation_fn|
