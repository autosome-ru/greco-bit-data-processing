...
module Chipseq
  SampleMetadata = Struct.new(*[
    ...
    def normalized_id
      ...
      id = (data_file_id != "") ? data_file_id : sample_label
      id && id.gsub(' ', '').sub(/\.fastq\.gz$/, "").sub(/_001$/, "").sub(/_R\d$/, "").sub(/_S\d+$/, "")
    end
  ...
  def self.verify_sample_metadata_match!
    ...
    sample_metadata_pairs = full_join_by(
      samples, metadata,
      key_proc_1: ->(fn){ File.basename(fn).split('.')[1].sub(/_L\d+(\+L\d+)?$/, "").sub(/_[ACGT]{6}$/, "").sub(/_S\d+$/, "") },
