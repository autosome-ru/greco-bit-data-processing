...
find -L /mnt/space/depla/smileseq_raw/ -xtype f -iname '*.fastq' | ruby -r fileutils -e '$stdin.readlines.map(&:chomp).each{|fn| new_bn = File.basename(fn).sub(/^UT(\d\d\d)_?(\d\d\d)_/, "UT\\1-\\2_"); FileUtils.ln_s(fn, "source_data/SMS/reads/unpublished/#{new_bn}") }'
