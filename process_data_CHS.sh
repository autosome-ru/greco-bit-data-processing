...
mkdir -p ${RESULTS_FOLDER}/validation_sequences/
for FN in $(find ${RESULTS_FOLDER}/validation_intervals/ -xtype f); do
  TF="$(basename -s .interval "$FN")"
  cat ${FN} | tail -n+2 | sort -k5,5nr | head -500 | ./bedtools getfasta -fi ./source_data/hg38.fa -bed - > ${RESULTS_FOLDER}/validation_sequences/${TF}.fa
done
...
for FN in $(find ${RESULTS_FOLDER}/train_intervals/ -xtype f ); do
    ...
    NEW_BN=$( ruby ${SCRIPT_FOLDER}/name_sample.rb "$FN" --slice-type Train --extension peaks )
    if [[ -n "$NEW_BN" ]]; then
...
for FN in $(find ${RESULTS_FOLDER}/validation_intervals/ -xtype f ); do
    # BN=$(basename -s .pbm.txt ${FN})
    NEW_BN=$( ruby ${SCRIPT_FOLDER}/name_sample.rb "$FN" --slice-type Val --extension peaks )
    if [[ -n "$NEW_BN" ]]; then
...
for FN in $(find ${RESULTS_FOLDER}/train_sequences/ -xtype f ); do
    # BN=$(basename -s .pbm.txt ${FN})
    NEW_BN=$( ruby ${SCRIPT_FOLDER}/name_sample.rb "$FN" --slice-type Train --extension fa )
    if [[ -n "$NEW_BN" ]]; then
...
for FN in $(find ${RESULTS_FOLDER}/validation_sequences/ -xtype f ); do
    # BN=$(basename -s .pbm.txt ${FN})
    NEW_BN=$( ruby ${SCRIPT_FOLDER}/name_sample.rb "$FN" --slice-type Val --extension fa )
    if [[ -n "$NEW_BN" ]]; then
